...
{
  FILE *result = fopen("ViberStringC1.txt", "w");
  int i, j;
  for (i = 0; i < row; i++)
  {
    for (j = 0; j < col; j++)
...
void Write2File1(float *A, int start, int end)
{
  FILE *result = fopen("ViberStringC1.txt", "a");
  int i, j;
  for (i = start; i < end; i++)
  {
      fprintf(result, "%lf ", *(A + start));
  }
  fclose(result);
}
...
int main()
{
  ...
  while (t <= T)
  {
    ...
    #pragma omp parallel
    {
      ...
      int start = id * m / numthread;
      int end = (id + 1) * m / numthread;
